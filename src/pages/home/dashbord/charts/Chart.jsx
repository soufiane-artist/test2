...
function Chart({lineSeriesCh,histogramSeriesCh,baselineSeriesCH,areaSeriesChart,BlueRibbon,userAdmin,value,setValue, setLoading ,setData,min,setArriv,arrivalDeal,setMont,barSeriesChart,Candel}) {
  ...
  const socket = useRef()
  useEffect(()=>{
    socket.current = io ('https://backend-lfin.onrender.com')
  ...
  return (
    <div>
      ...
      <OpenDealphone setLoading={setLoading} value={value} setMont={setMont} socket={socket} arrivalDeal={arrivalDeal} setArriv={setArriv} dataSidibarRigth={dataSidibarRigth} setDialNotif={setDialNotif} dealValue={dealValue}  setdealValue={setdealValue} />
      {dealNotif &&<DealNotif/>}
      </div> 
    </div>
  );
